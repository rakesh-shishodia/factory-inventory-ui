...
**Current production status (2026-09-22): protected login works; inventory is not live.** The HTTPS login-test deployment has Cloudflare Access email-code login and an exact approved staff allowlist. The user has confirmed a successful approved-user login. All three inventory/order/stock-sync flags remain false, no Ecwid secrets or cron have been configured in production, and the production database has eight verified schema migrations but no business rows. No real pilot items have been staged or activated, and no pilot alignment has changed Ecwid. Empty counters mean **not imported**, not zero factory stock. Actual factory-phone scanning, picking and receiving remain untested.

The local implementation includes the mobile app, variation-aware stock, supplier receiving/allocation, exact opening-order staging, reviewed workbook/app mixed orders, a private cutover request assembler and a guarded operator CLI. Recent-update polling and extended workbook-only profile option policies are implemented and tested locally, but the updated Worker code is not yet deployed. Migration `0008` has been applied and verified in production after backup and a populated local runtime rehearsal. The alignment service has no public HTTP endpoint. See the [pilot cutover runbook](docs/pilot-cutover.md) for the remaining launch gates; a schema upgrade or source-code availability is not a completed cutover.
...
`npm run cutover:operate -- --help` describes the private operator: `status`, `preview`, `stage`, `begin`, `row`, `rows` and `finish`. Its private configuration pins the Cloudflare account, Worker, database, store, administrator and deployed version. Authentication is obtained through official Wrangler tooling without printing its token. The `rows` command reuses one database connection, discovers only the exact immutable staged batch, skips verified rows and stops at the first held row/error. Final activation remains a separate command. `status` is read-only recovery information, not permission to repeat a stock write. See [the runbook](docs/pilot-cutover.md#private-operator-configuration) for the configuration and staged command sequence.
...
1. Deploy and verify the reviewed profile-policy, recent-order-feed and UI code with all three flags still false. Migration `0008` is already applied: verify the eight-entry production journal rather than rerunning it. The existing isolated production resources and Access login need verification, not recreation. Keep queue `max_batch_size:1` before enabling processing; multiple messages can exceed a Free Worker invocation's budget. Keep cron disabled until the cutover is active.
...
3. Configure the Ecwid secret token through a Wrangler secret prompt only when ready for live integration. Verify `read_catalog`, `update_catalog` and `read_orders` for the intended store. `ECWID_CLIENT_SECRET` and an exact-path Access bypass are required only if signed webhooks are configured; never bypass the rest of the app. Keep inventory recording, live stock sync and order sync false throughout preparation.
...
5. Confirm a real pause of pilot checkout and every pilot movement/competing integration. Refresh the workbook and complete catalogue/order snapshots during the freeze; assemble and preview the exact request, then stage it atomically. The entire guarded alignment must fit the 15-minute window. Never change timestamps, bypass the clock or start a replacement batch to escape an expired/uncertain existing one.
