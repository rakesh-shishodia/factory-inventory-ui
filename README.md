...
**Current delivery: a working local app with supplier-backed receiving/allocation/picking, variation-aware integration, exact opening-order staging, and explicitly reviewed workbook/app mixed orders. The picker is not operating the real store.** No real pilot items have been imported, no pilot alignment has changed Ecwid, and no operational mobile link has been published. Cloudflare Access and the approved staff allowlist still need production configuration; the staff approval record is private, not committed. Timely Ecwid order webhooks are also required before floor launch with the current polling implementation. The guarded one-time alignment/activation service is separate from the staging routes; it has no public HTTP route or operator CLI. See the [pilot cutover runbook](docs/pilot-cutover.md) before any real migration.
...
- A guarded alignment journal and separate operator service for fresh preflight, one-attempt stock alignment, verification and activation. No public alignment endpoint or operator CLI is exposed.
...
Store ID: **2442119**. Keep the Worker in demo mode with `INVENTORY_ENABLED=false`, `LIVE_SYNC_ENABLED=false` and `ORDER_SYNC_ENABLED=false` during this stage. The local connection check uses a separate `.env.ecwid-readonly` file; it does not start the Worker, import orders, write the database or change stock.
...
The same command also writes `opening-orders.json`, a complete `READONLY_ORDERS` envelope for the new cutover importer. It includes normalized full pending-order line identities, explicit digital/quantity-tracking evidence, collection interval/cutoff, checked-order count and pending order/line counts. Supported option selections are canonicalized; unsupported selections are redacted and remain blocked. This file is evidence, not approval: staging still requires an explicit zero-previous-pick confirmation for every exact line, including workbook-managed lines.
...
2. Protect the app with Cloudflare Access. Configure `ACCESS_TEAM_DOMAIN` (full `https://…cloudflareaccess.com` URL), `ACCESS_AUD`, comma-separated exact `STAFF_EMAILS`, and `ADMIN_EMAILS`. Administrators must also appear in `STAFF_EMAILS`; a valid Access token alone grants no app access. Keep only `/api/webhooks/ecwid` outside browser sign-in; its Ecwid signature is verified by the Worker. Production Access is not yet enabled for this pilot account.
3. Set `ECWID_MODE=live`, `ECWID_STORE_ID`, and secrets `ECWID_TOKEN` and `ECWID_CLIENT_SECRET` through Wrangler's secret prompts. Use Ecwid scopes `read_catalog`, `update_catalog`, and `read_orders`. Keep `INVENTORY_ENABLED=false`, `LIVE_SYNC_ENABLED=false` and `ORDER_SYNC_ENABLED=false` during preparation; all default to false. Order refresh, polling and webhook processing remain paused while order sync is off.
4. Validate the approved 76 stock-limited matches and explicit workbook-managed exclusions. The six MISUMI supplier-backed targets are excluded from this alignment, not assigned guessed opening quantities. Prepare backups and production access before asking staff to pause. Resolve unknown mappings and any already-picked opening lines; this first cutover importer requires zero previous picks per line and cannot invent history.
5. Briefly freeze pilot checkout, picking, receipts and other stock movements; disable competing legacy writes for the pilot. Collect fresh workbook/catalogue/full-order evidence during the freeze, confirm exact lines and counts, review `/api/cutover/preview`, then stage the identical hash-bound request in the clean production database. Do not turn on order imports before the mapping and opening-order transaction is complete. If the 15-minute window expires, stop and obtain fresh approval/evidence rather than using stale snapshots.
6. Use a reviewed operator integration of the guarded alignment service to set Ecwid once to `workbook Balance − unpicked commitments`, read every target back, revalidate complete pending orders and activate only the verified batch. No operator CLI or HTTP alignment route currently exists. Activation preserves the original freeze timestamp as the order-tracking baseline. Configure and verify signed Ecwid order webhooks before floor launch, with an Access bypass restricted to the exact `/api/webhooks/ecwid` path; browser/API access stays protected. Enable `ORDER_SYNC_ENABLED`, inventory recording and live delta delivery only after successful activation and deployment checks; confirm a full order scan and resolve any new issues before resuming activity.
...
**The five-minute cron is not five-minute order freshness.** Current polling advances only two 100-order pages per run through the full history. At the reviewed 8,298 historical orders, a complete cycle takes approximately 42 runs, or 210 minutes at the configured interval. New orders after the scan's fixed cutoff may wait for another cycle. This is a reconciliation fallback, not an adequate sole source of timely reservations. Configure and test Ecwid order webhooks before factory-floor launch, or implement and verify recent-update polling first. Exact order lookup alone does not keep all other commitments current.
...
| `src/cutover-alignment.ts`, `docs/pilot-cutover.md` | Guarded one-time alignment/activation service and operator runbook |
