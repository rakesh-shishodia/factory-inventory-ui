...
**Current production status (2026-09-22): the reduced worker workflow is enabled for a supervised factory-floor pilot.** `INVENTORY_ENABLED`, `LIVE_SYNC_ENABLED` and `ORDER_SYNC_ENABLED` are true in deployed version `12aff444-e022-487c-a2c3-db6210ae76a1`. A five-minute schedule is configured only to recover already committed stock-outbox work; it never polls orders. Cloudflare Access remains in place with email-code login, the exact approved staff allowlist and the one-month session. One supervised approved Paid-order pick and one restock, with database and Ecwid verification after each, are still required before normal floor use.

Enabling the supervised pilot did not rerun or overwrite the completed cutover. All 76 approved stock-limited pilot targets remain active and aligned at workbook Balance minus exact unpicked commitments; the seven opening orders and 38 lines remain loaded. The six MISUMI targets and the other explicitly registered exclusions remain workbook-managed. A private pre-upgrade backup was captured, and `0009_simple_movement_safety.sql` was applied and verified as the ninth production migration without changing any balance, movement, outbox entry or issue.
...
## Production pilot enabled; reduced floor acceptance still required
...
4. Verify the intended store and token permissions (`read_catalog`, `update_catalog`, `read_orders`). Keep secrets in the Worker/private operator only. The reduced one-screen UI was reviewed on the protected production link before movements were enabled.
5. The supervised smoke window was enabled in version `12aff444-e022-487c-a2c3-db6210ae76a1`; inventory, live stock sync and exact staff-requested order refresh are active. This does not enable a background order feed. Keep the configured five-minute outbox-only recovery schedule.
6. On the actual factory phone, test scanner open/close/camera switch and manual SKU entry, then one approved Paid-order pick and one restock. Verify D1 physical stock, reservation change and the applicable Ecwid result before normal floor use. Pause the three flags and reconcile immediately on any mismatch or uncertain outbox result.
