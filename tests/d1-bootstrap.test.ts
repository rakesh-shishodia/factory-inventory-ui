...
describe('empty-only D1 bootstrap artifact',()=>{
  it('preserves all seven migrations byte-for-byte with deterministic names and the normal Wrangler journal',async()=>{
    const input=await sources();expect(input).toHaveLength(7);
    ...
    try {
      ...
      expect(journal.map(row=>row.id)).toEqual([1,2,3,4,5,6,7]);expect(journal.every(row=>Number.isFinite(Date.parse(row.applied_at)))).toBe(true);
  ...
  it('allows only an empty normal journal and Cloudflare internal table',async()=>{
    ...
    try {
      ...
      expect(db.prepare('SELECT COUNT(*) AS n FROM d1_migrations').get()).toEqual({n:7});
